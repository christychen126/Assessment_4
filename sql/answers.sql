...
SELECT b.name, m.name, m.year, b.discontinued, (b.discontinued-m.year) AS years_until_brand_is_discontinued FROM models AS m LEFT JOIN brands AS b ON m.brand_name=b.name WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT m.name, m.year FROM models AS m JOIN brands AS b ON m.year = b.founded;